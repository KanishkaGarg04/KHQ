...
function MetricCard({ m, i }: { m: (typeof METRICS)[number]; i: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="h-1.5 w-1.5 rounded-full animate-pulse-glow" style={{ background: m.color, boxShadow: `0 0 8px ${m.color}` }} />
      </div>
      <div className="mt-3 font-display text-2xl font-semibold tabular-nums">{n.toLocaleString()}</div>
      <div className="mt-0.5 font-mono text-[9px] uppercase tracking-[0.2em] text-muted-foreground">{m.label}</div>
    </motion.div>
  );
}
...
export function Hero() {
  ...
  return (
    <section ref={wrap} className="relative min-h-screen w-full overflow-hidden">
      <div
      ...
      <header className="relative z-30 flex items-center justify-between px-6 pt-6 md:px-12">
        <div className="flex items-center gap-2 font-display text-sm">
          ...
          <span className="font-mono uppercase tracking-[0.2em] text-muted-foreground">KAN/LABS · HQ</span>
        </div>
        <nav className="hidden gap-5 text-[11px] uppercase tracking-[0.18em] text-muted-foreground lg:flex">
          {NAV.map((m) => (
            <button key={m.id} onClick={() => scrollTo(m.id)} className="transition-colors hover:text-foreground">
              {m.label}
            </button>
          ))}
        </nav>
        ...
        <div className="relative">
          ...
          <div className="relative mx-auto aspect-square w-full max-w-[420px]">
            ...
            <div
              ...
              style={{
                background: "conic-gradient(from 0deg, transparent 0deg, var(--neon-cyan) 30deg, transparent 60deg)",
                opacity: 0.18,
            ...
            {[
              ...
              <motion.span
                ...
                style={{ left: `${b.x}%`, top: `${b.y}%`, background: b.c, boxShadow: `0 0 10px ${b.c}` }}
            ...
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
              <div
                ...
                style={{ background: "var(--gradient-holo)", boxShadow: "var(--glow-cyan)" }}
              >
                <span className="font-display text-2xl font-bold text-background">KG</span>
              </div>
            </div>
          </div>
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 pb-12 md:px-8">
        <div className="glass flex flex-wrap items-center justify-between gap-3 rounded-2xl px-5 py-3 font-mono text-[10px] uppercase tracking-[0.25em] text-muted-foreground">
          ...
          <span className="hidden md:inline">Last commit · 2h ago · feat(hero): live mission dashboard</span>
          <span className="flex items-center gap-2">
